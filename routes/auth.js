...
router.post('/checkUsernameAvailability', (req, res) => {
    if (req.body.username) {
        User.findOne({ username: req.body.username })
            .then(user => {
                if (user) {
                    ...
                    res.json({ success: false, data: { msg: 'Username already taken', usernameAvailable: false } });
                } else {
                    res.json({ success: true, data: { msg: 'Username available', usernameAvailable: true } })
                }
            })
            .catch(err => {
        ...
        res.json({ success: false, data: { msg: 'Bad request. Please check all your fields' } });
    }
});
...
router.post('/signup', (req, res) => {
    if (req.body.username && req.body.password) {
        User.findOne({ username: req.body.username })
            .then(user => {
                if (user) {
                    ...
                    res.json({ success: false, data: { msg: 'Username already exists' } });
                } else {
                        ...
                        if (err) {
                            ...
                            user.save(err => {
                                if (err) {
                                    ...
                                    res.json({ success: true, data: { msg: 'Account created successfully' } });
                                }
                            })
                        }
                    })
        ...
        res.json({ success: false, data: { msg: 'Bad request. Please check your fields' } });
    }
})
